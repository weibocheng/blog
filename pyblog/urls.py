"""pyblog URL Configuration

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/2.0/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path, re_path
from django.views.static import serve

from blog import views
from pyblog import settings


urlpatterns = [
    path('admin/', admin.site.urls),
    path('login/', views.login),
    path('logout/', views.logout),
    path('check_code/', views.check_code),
    path('register/', views.register),
    path('create_blog/', views.create_blog),
    # 评论
    path('comment/', views.comment),
    path('del_comment/', views.del_comment),
    path('comment_tree/', views.comment_tree),

    # 点赞
    path('up_down/', views.up_down),

    # 后台管理
    path('backend/', views.backend),
    path('add_article/', views.add_article),
    path('delete_article/', views.delete_article),
    path('add_tag/', views.add_tag),
    path('add_category/', views.add_category),
    re_path(r'^edit_article/(?P<article_id>\d+)/$', views.add_article),


    # 文本编辑器上传图片
    path('upload/', views.upload),

    # media配置:
    re_path(r"^media/(?P<path>.*)/$", serve, {"document_root": settings.MEDIA_ROOT}),

    # re_path(r"^(?P<username>\w+)/articles/(?P<article_id>\d+)$", views.article_detail),
    # article_detail(request,username="yuan","article_id":article_id)
    # 个人主站
    re_path(r'^(?P<site>\w+)/$', views.home),
    re_path(r'^(?P<site>\w+)/(?P<condition>tag|category|archive)/(?P<param>.*)/$', views.home),
    # 文章详细
    re_path(r'^(?P<site>\w+)/articles/(?P<article_id>\d+)/$', views.article_detail),


    re_path('^$', views.index),
]
